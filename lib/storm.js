...
Storm.prototype = {
	...
	tupleCallback: function(tuple, self) {
		...
		self.log("Process tuple here: " + tuple.tuple,self);
	...
	emitStream: function(tuple, stream , self) {
		self.emitTuple(tuple, stream, [], null, self)
	},
	...
	parseMessage: function(msg, self) {
		...
		if(self.msgCount == 1) {
			...
			self.initCallback(data.config,data.context,self);
		} else {
			tuple = {};
